...
def _append_aligned(buffer: bytearray, payload: bytes) -> Tuple[int,int]:
    ...
    offset = len(buffer); buffer.extend(payload); return offset, len(payload)

def build_semantic_glb(source: GovernedSynthesisPackage, semantic: SemanticGeometryPackage):
    validate_package(source); validate_semantic_geometry(semantic, source)
    if len(source.geometry.primitives) != len(semantic.objects): raise SemanticGLBError("source/semantic primitive count mismatch")
    sem_by_primitive = {row.primitive_id: row for row in semantic.objects}
    binary=bytearray(); buffer_views=[]; accessors=[]; meshes=[]; nodes=[]; materials=[]; material_by_class:Dict[str,int]={}
    class_colors={"ADMITTED_SPATIAL_ENVELOPE":[0.45,0.70,0.90,1.0],"STRUCTURAL_HYPOTHESIS":[0.82,0.82,0.82,1.0],"SOURCE_NODE_PROXY":[0.95,0.62,0.28,1.0]}
    for primitive in source.geometry.primitives:
        sem=sem_by_primitive[primitive.primitive_id]; vertices=tuple(_finite_vec3(v) for v in primitive.vertices_m)
        flat=[c for v in vertices for c in v]; pb=struct.pack("<"+"f"*len(flat),*flat); po,pl=_append_aligned(binary,pb); pv=len(buffer_views); buffer_views.append({"buffer":0,"byteOffset":po,"byteLength":pl,"target":34962})
        pa=len(accessors); accessors.append({"bufferView":pv,"byteOffset":0,"componentType":5126,"count":len(vertices),"type":"VEC3","min":[min(v[i] for v in vertices) for i in range(3)],"max":[max(v[i] for v in vertices) for i in range(3)]})
        inds=[int(idx) for tri in primitive.triangles for idx in tri]; ib=struct.pack("<"+"I"*len(inds),*inds); io,il=_append_aligned(binary,ib); iv=len(buffer_views); buffer_views.append({"buffer":0,"byteOffset":io,"byteLength":il,"target":34963})
        ia=len(accessors); accessors.append({"bufferView":iv,"byteOffset":0,"componentType":5125,"count":len(inds),"type":"SCALAR","min":[min(inds)],"max":[max(inds)]})
        if sem.semantic_class not in material_by_class:
            material_by_class[sem.semantic_class]=len(materials); materials.append({"name":sem.semantic_class,"doubleSided":True,"pbrMetallicRoughness":{"baseColorFactor":class_colors.get(sem.semantic_class,[0.7,0.7,0.7,1.0]),"metallicFactor":0.0,"roughnessFactor":0.9}})
        mi=len(meshes); meshes.append({"name":primitive.primitive_id,"primitives":[{"attributes":{"POSITION":pa},"indices":ia,"material":material_by_class[sem.semantic_class],"mode":4}]})
    ...
    jc=_pad4(_canonical_json_bytes(gltf),b" "); bc=_pad4(bytes(binary),b"\x00"); total=12+8+len(jc)+8+len(bc); glb=struct.pack("<4sII",b"glTF",2,total)+struct.pack("<II",len(jc),0x4E4F534A)+jc+struct.pack("<II",len(bc),0x004E4942)+bc
...
def canonical_manifest_json(manifest: dict) -> str:
    if manifest.get("authority_status") != SEMANTIC_GLB_AUTHORITY: raise SemanticGLBError("GLB manifest authority escalation")
    if manifest.get("flight_dynamics_authority") or manifest.get("canon_changed") or manifest.get("production_shipclasses_changed"): raise SemanticGLBError("GLB manifest may not mutate frozen authority")
