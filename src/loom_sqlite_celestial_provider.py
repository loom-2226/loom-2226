"""Read-only SQLite adapter into the shared celestial-state authority.

This is the production-facing data seam promoted from the prior spatial lineage.
It consumes existing celestial tables; it does not create, mutate, or reinterpret
renderer-only fallback data as navigation authority.
"""
...
def osculating_elements_from_state(
    ...
    ecc = _norm(evec)
    if not 0.0 <= ecc < 1.0:
        raise SQLiteCelestialCatalogError(f"only bound elliptic anchors are supported; e={ecc}")
    energy = 0.5 * vmag * vmag - mu / rmag
    if energy >= 0.0:
        raise SQLiteCelestialCatalogError("ephemeris anchor is not bound")
    a = -mu / (2.0 * energy)
    inc = math.acos(_clamp(h[2] / hmag))
    raan = math.atan2(n[1], n[0]) % (2.0 * math.pi) if nmag > 1e-12 else 0.0
    if ecc > 1e-12 and nmag > 1e-12:
...
class SQLiteCelestialCatalog:
    """Read-only provider of direct states and propagated child models."""
    ...
    def direct_state(self, entity_id: str, epoch_utc: str) -> SpatialState | None:
        """Return only an exact, navigation-grade canonical inertial state."""
        epoch = _iso(_epoch(epoch_utc))
        with self._connect() as conn:
            row = conn.execute(
                """
                SELECT entity_id,x_au,y_au,z_au,vx_au_d,vy_au_d,vz_au_d,source,navigation_grade
                FROM states
        ...
        return SpatialState(
            ...
            velocity_km_s=(
                row["vx_au_d"] * AU_KM / DAY_S,
                row["vy_au_d"] * AU_KM / DAY_S,
                row["vz_au_d"] * AU_KM / DAY_S,
            ),
    ...
    def orbit_model_from_direct_anchor(self, entity_id: str, requested_epoch_utc: str) -> ParentCentricOrbitModel:
        ...
        with self._connect() as conn:
            ...
            if anchor is None:
                raise SQLiteCelestialCatalogError(f"no genuine navigation-grade parent-centric anchor for {entity_id}")
