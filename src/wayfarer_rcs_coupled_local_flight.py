...
def simulate_coupled_case(*, name: str, target_velocity_m_s: list[float], axis: str, angle_deg: float, failed_cluster: str | None) -> dict[str, Any]:
    ...
    while t < MAX_TIME_S:
        ...
        if terminal_velocity_error <= VELOCITY_GATE_M_S and terminal_attitude_error <= ATTITUDE_GATE_DEG and terminal_rate <= RATE_GATE_DEG_S:
            in_gate += DT_S
    ...
    terminal_attitude_error = _attitude_error_deg(q, target_q)
    terminal_rate = math.degrees(math.sqrt(_dot(omega, omega)))
    passed = in_gate + 1e-12 >= HOLD_TIME_S and terminal_velocity_error <= VELOCITY_GATE_M_S and terminal_attitude_error <= ATTITUDE_GATE_DEG and terminal_rate <= RATE_GATE_DEG_S and peak_force_fraction <= 1.0 + 1e-9 and peak_torque_fraction <= 1.0 + 1e-9
    return {"case": name, "failed_cluster": failed_cluster, "target_velocity_m_s": list(target_velocity_m_s), "terminal_velocity_m_s": velocity, "terminal_position_offset_m": position, "terminal_velocity_error_m_s": terminal_velocity_error, "target_attitude_axis": axis, "target_attitude_angle_deg": float(angle_deg), "terminal_attitude_error_deg": terminal_attitude_error, "terminal_body_rate_deg_s": terminal_rate, "duration_s": t, "force_limit_N": force_limit, "torque_limits_Nm": torque_limits, "peak_force_fraction": peak_force_fraction, "peak_torque_fraction": peak_torque_fraction, "mass_t": state["mass_t"], "center_of_mass_m": state["center_of_mass_m"], "inertia_tensor_kg_m2": tensor, "dynamics_model": "COUPLED_TRANSLATION_PLUS_FULL_RIGID_BODY_ROTATION", "translation_model": "NET_INERTIAL_FORCE_VELOCITY_TRACKING_WITH_FORCE_SATURATION", "rotation_model": "QUATERNION_PD_FULL_EULER_DYNAMICS_WITH_TORQUE_SATURATION", "pass": passed}


def _qualify(cases: dict[str, dict[str, Any]], failed_cluster: str | None) -> dict[str, Any]:
    results = {name: simulate_coupled_case(name=name, failed_cluster=failed_cluster, **spec) for name, spec in cases.items()}
...
def build_coupled_local_flight_qualification() -> dict[str, Any]:
    ...
    return {"schema": SCHEMA, "schema_version": SCHEMA_VERSION, "nominal": nominal, "one_cluster_out": degraded, "all_cases_pass": all_pass, "disposition": "COUPLED_TRANSLATION_ROTATION_NET_WRENCH_ENGINEERING_QUALIFIED" if all_pass else "COUPLED_TRANSLATION_ROTATION_NET_WRENCH_ENGINEERING_FAIL", "authority": {"coupled_translation_rotation_time_domain_simulated": True, "configuration_aware_mass_inertia_consumed": True, "net_force_torque_envelope_consumed": True, "mount_level_allocator_in_time_loop": False, "minimum_impulse_bit_certified": False, "plume_interference_certified": False, "structural_mount_loads_certified": False, "final_flight_software_certified": False, "final_thruster_hardware_certified": False, "canon_changed": False, "campaign_state_mutation": "ZERO", "llm_calculation_authority": "ZERO"}, "remaining_open": ["mount_level_force_allocation_in_the_time_loop", "minimum_impulse_bit_and_valve_gimbal_dynamics", "finite_plume_and_external_hardware_interference", "structural_mount_loads"]}
