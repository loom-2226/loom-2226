...
def _file_identity(path: Path) -> dict[str, Any]:
    exists = path.exists()
    return {"path": str(path), "exists": exists, "is_file": path.is_file(), "size_bytes": path.stat().st_size if path.is_file() else None, "sha256": _sha256(path)}


def _git_head(app_root: Path) -> str | None:
    try:
        proc = subprocess.run(["git", "-C", str(app_root), "rev-parse", "HEAD"], capture_output=True, text=True, timeout=2, check=False)
...
def collect_runtime_manifest(*, roots: RuntimeRoots | None = None) -> dict[str, Any]:
    ...
    for root_name, root in (("data_root", data), ("app_data", app / "data")):
        for name in db_names:
            ...
            item.update(root=root_name, name=name)
    ...
    trace_path = campaign / "logs" / "loom-trace.jsonl"
    return {
        "contract": CONTRACT,
        "runtime": {"platform": platform.platform(), "python": sys.version.split()[0], "executable": sys.executable, "cwd": str(Path.cwd().resolve()), "git_head": _git_head(app)},
        "roots": roots.to_dict(),
        "environment": {key: os.environ.get(key) for key in ("LOOM_APP_ROOT", "LOOM_DATA_ROOT", "LOOM_CAMPAIGN_ROOT", "LOOM_HOME")},
        "source": {"loom_gis": _file_identity(app / "src" / "loom_gis.py"), "runtime": _file_identity(app / "src" / "loom" / "runtime.py")},
        "databases": databases,
        "campaign": {
            "state": campaign_state,
            "navigator_cache": {"path": str(campaign / "LOOM_Navigator_Cache_v1"), "exists": (campaign / "LOOM_Navigator_Cache_v1").exists()},
        },
        "observability": {"trace": _file_identity(trace_path)},
    }


def render_runtime_audit(manifest: dict[str, Any]) -> str:
    roots, runtime = manifest["roots"], manifest["runtime"]
    ...
    trace = manifest["observability"]["trace"]
    lines.extend(["", f"CAMPAIGN STATE {'PRESENT' if state['exists'] else 'missing'} {state['path']}", f"TRACE          {'PRESENT' if trace['exists'] else 'missing'} {trace['path']}"])
    return "\n".join(lines) + "\n"
