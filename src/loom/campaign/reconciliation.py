...
def reconcile_campaign_shadow(campaign_root:Path|str, core:Any)->dict[str,Any]:
    ...
    root=Path(campaign_root).expanduser().resolve(); state=_state(root,core)
    ledger_type=getattr(core,"HistoryLedger",None)
    if ledger_type is None:raise CampaignShadowReconciliationError("canonical HistoryLedger unavailable")
    # Construction invokes the canonical ledger's schema, numbering, hash-chain,
    # archive, active-history and backup-recovery verification machinery.
    ...
    if not shadow.path.is_file():return {**base,"status":"MISSING_DB","match":False,"shadow_rows":0,"missing_in_sql":[str(r.get("flight_id") or "") for r in arrivals],"extra_in_sql":[],"mismatches":[],"revision_gaps":[]}
    try:
        uri=f"file:{shadow.path.as_posix()}?mode=ro"
        with sqlite3.connect(uri,uri=True) as conn:
            rows=conn.execute("SELECT flight_id,revision_before,revision_after,state_before_id,state_after_id,history_record_number,history_record_sha256,canonical_commit_json FROM campaign_flight_commits ORDER BY revision_after,flight_id").fetchall()
    ...
    for fid in sorted(set(canonical)&set(sql)):
        h=canonical[fid]; s=sql[fid]
        try:commit=json.loads(s[7])
        except Exception:commit={}
        expected={"history_record_number":int(h.get("record_number",-1)),"history_record_sha256":str(h.get("record_sha256") or ""),"state_before_id":h.get("state_before_id"),"state_after_id":h.get("state_after_id")}
        actual={"history_record_number":int(s[5]),"history_record_sha256":str(s[6]),"state_before_id":s[3],"state_after_id":s[4]}
        # Revision is authoritative in the state snapshots carried by the verified
        # history record. Compare it when present; do not infer absent values.
        before=h.get("state_before_snapshot") or {}; after=h.get("state_after_snapshot") or {}
        if isinstance(before,dict) and before.get("revision") is not None: expected["revision_before"]=int(before["revision"]); actual["revision_before"]=int(s[1])
        if isinstance(after,dict) and after.get("revision") is not None: expected["revision_after"]=int(after["revision"]); actual["revision_after"]=int(s[2])
        if expected!=actual or str(commit.get("flight_id") or "")!=fid:mismatches.append({"flight_id":fid,"canonical":expected,"shadow":actual})
    revisions=sorted(int(r[2]) for r in rows); gaps=[]
    for left,right in zip(revisions,revisions[1:]):
        if right!=left+1:gaps.append([left,right])
    latest_sql=max(revisions) if revisions else None; current_revision=state.get("revision"); state_aligned=(latest_sql is None and not arrivals) or (latest_sql is not None and current_revision is not None and int(latest_sql)==int(current_revision))
    match=not missing and not extra and not mismatches and not gaps and state_aligned
