...
def _canonical_navigation_endpoint(registry_owner: Any, candidates: Iterable[str]) -> tuple[str | None, str | None]:
    ...
    for raw_token, raw_entity in registry.items():
        token, entity_id = str(raw_token or "").strip().upper(), str(raw_entity or "").strip().upper()
...
def install_flight_planning(gis_module: Any, session: GISFlightPlanningSession) -> None:
    ...
    handler.flight_planning_job = {"status": "IDLE", "job_id": None, "state": None, "error": None, "trace_id": None}
    runtime_root = Path(getattr(session.context, "runtime_root", None) or Path.cwd())
    roots = resolve_runtime_roots(app_root=runtime_root)
    handler.runtime_trace = RuntimeTrace(roots.campaign_root / "logs" / "loom-trace.jsonl")
    handler.active_trace_id = None
    ...
    try: latest_pointer.write_text(f"HTTP_LOG={handler.flight_planning_log_path.name}\nTRACE_LOG={handler.runtime_trace.path}\n", encoding="utf-8")
    except Exception: pass
    old_get, old_post = handler.do_GET, getattr(handler, "do_POST", None)
    ...
    def _trace(event_type: str, trace_id: str | None = None, **fields: Any) -> str:
        tid = trace_id or handler.active_trace_id or handler.runtime_trace.new_trace_id("flight")
        handler.active_trace_id = tid
        handler.runtime_trace.emit(event_type, trace_id=tid, subsystem="gis.flight_planning", **fields)
        return tid

    _log("flight_planning_installed", log_path=str(handler.flight_planning_log_path), runtime_root=str(runtime_root))
    _trace("runtime.installed", runtime_root=str(runtime_root), trace_path=str(handler.runtime_trace.path))
    ...
    def _redirect(self, location: str, status: int = 303):
        ...
        self.send_response(status); self.send_header("Location", location); self.send_header("Content-Type", "text/plain; charset=utf-8"); self.send_header("Content-Length", str(len(body))); self.send_header("Cache-Control", "no-store"); self.end_headers(); self.wfile.write(body)
    def _job_snapshot():
    ...
    def _set_job(**updates):
        with handler.flight_planning_job_lock:
            handler.flight_planning_job.update(updates); return dict(handler.flight_planning_job)

    def _resolve_destination(entity: Any, trace_id: str | None = None) -> dict[str, Any]:
        ...
        if token is None:
            _trace("destination.unavailable", trace_id, entity_id=entity.get("entity_id"), display_name=display_name)
        ...
        _trace("destination.resolved", trace_id, entity_id=result.get("entity_id"), route_token=token, selectable=result["selectable"])
        return result

    def _run_discovery_job(job_id: str, destination: str, priority: str, trace_id: str):
        _log("discover_job_start", job_id=job_id, destination=destination, priority=priority, trace_id=trace_id); _trace("discover.started", trace_id, job_id=job_id, destination=destination, priority=priority)
        try:
            state = handler.flight_planning_session.discover(destination, priority)
            _set_job(status="COMPLETE", job_id=job_id, state=state.to_dict(), error=None, trace_id=trace_id)
            _trace("discover.completed", trace_id, job_id=job_id, destination=destination, candidate_count=len(state.candidates)); _log("discover_job_complete", job_id=job_id, candidates=len(state.candidates), trace_id=trace_id)
        except BaseException as exc:
            detail = str(getattr(exc, "code", None)) if isinstance(exc, SystemExit) else str(exc)
            message = f"Navigator acquisition exited: {detail}" if isinstance(exc, SystemExit) else f"{type(exc).__name__}: {exc}"
            _trace("discover.failed", trace_id, job_id=job_id, destination=destination, error=message); _log("discover_job_error", job_id=job_id, error=message, trace_id=trace_id, traceback=traceback.format_exc()); _set_job(status="ERROR", job_id=job_id, state=None, error=message, trace_id=trace_id)

    def _start_discovery(destination: str, priority: str):
        current = _job_snapshot()
        if current.get("status") == "RUNNING": return current, 409
        job_id, trace_id = "fp-" + uuid.uuid4().hex[:12], handler.runtime_trace.new_trace_id("flight")
        handler.active_trace_id = trace_id
        _set_job(status="RUNNING", job_id=job_id, state=None, error=None, trace_id=trace_id)
        _trace("discover.accepted", trace_id, job_id=job_id, destination=destination, priority=priority); _log("discover_accepted", job_id=job_id, destination=destination, priority=priority, trace_id=trace_id)
        threading.Thread(target=_run_discovery_job, args=(job_id, destination, priority, trace_id), name=f"loom-flight-plan-{job_id}", daemon=True).start()
    ...
    def do_GET(self):
        ...
        if path == "/diagnostics":
            ...
            if client not in ("127.0.0.1", "::1", "localhost"): return _send_json(self, {"error": "diagnostics is localhost-only"}, status=403)
            manifest = collect_runtime_manifest(roots=roots)
            manifest["live_process"] = {"flight_planning": self.flight_planning_session.state().to_dict(), "job": _job_snapshot(), "http_log": str(handler.flight_planning_log_path), "trace_log": str(handler.runtime_trace.path), "active_trace_id": handler.active_trace_id}
            return _send_json(self, manifest)
        if path == "/flight-planning.json": return _send_state(self, self.flight_planning_session.state())
        if path == "/flight-planning/status.json": return _send_json(self, _job_snapshot())
        if path == "/flight-planning/health.json": return _send_json(self, {"ok": True, "planning": self.flight_planning_session.state().to_dict(), "job": _job_snapshot(), "log_path": str(handler.flight_planning_log_path), "trace_id": handler.active_trace_id})
        if path in ("/flight-planning/discover.json", "/flight-planning/discover-start"):
            ...
            if not destination: return _redirect(self, "/?fp_error=destination_required") if path.endswith("discover-start") else _send_json(self, {"error": "destination is required"}, status=400)
    ...
    def do_POST(self):
        ...
        trace_id = str(self.headers.get("X-LOOM-Trace-ID") or handler.active_trace_id or handler.runtime_trace.new_trace_id("flight"))
        handler.active_trace_id = trace_id
        _trace("http.request", trace_id, method="POST", path=path)
        try:
            n = int(self.headers.get("Content-Length") or 0); raw = self.rfile.read(n) if n > 0 else b"{}"; body = json.loads(raw.decode("utf-8") or "{}")
            ...
            else: return _send_json(self, {"error": "unknown flight-planning endpoint"}, status=404)
            action = path.rsplit("/", 1)[-1]
            state_dict = state.to_dict()
            _trace(f"planning.{action}.completed", trace_id, route_id=body.get("route_id"), destination=body.get("destination"), state=state_dict)
            _log("planning_action_complete", path=path, trace_id=trace_id, state=state_dict)
            return _send_state(self, state)
        except (GISFlightPlanningError, ValueError, KeyError, RuntimeError) as exc:
            _trace("planning.failed", trace_id, path=path, error=f"{type(exc).__name__}: {exc}"); _log("planning_action_error", path=path, trace_id=trace_id, error=f"{type(exc).__name__}: {exc}", traceback=traceback.format_exc()); return _send_json(self, {"error": f"{type(exc).__name__}: {exc}"}, status=400)

    handler.do_GET, handler.do_POST = do_GET, do_POST
