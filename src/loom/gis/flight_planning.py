...
class GISFlightPlanningSession:
    ...
    def _bind_context(self,context):
        ...
        self.origin=origin; self.destination=None; self.priority="BALANCED"; self._request=None; self._candidates={}; self._plans={}; self.preview_route_id=None; self.committed_route_id=None; self.preview_overlay=None; self.gravity_shadow_report=None; seed={"state":self._campaign_before.get("state_id"),"revision":self._campaign_before.get("revision"),"origin":origin}; self.session_id="plan-"+hashlib.sha256(_stable_json(seed)).hexdigest()[:16]; self.campaign_state_sha256=hashlib.sha256(_stable_json(self._campaign_before)).hexdigest()
    ...
    def state(self):return GISPlanningStateV1(session_id=self.session_id,origin=self.origin,destination=self.destination,priority=self.priority,candidates=tuple(GISPlanningCandidateV1(c.route_id,_candidate_summary(c),dict(c.payload)) for c in self._candidates.values()),preview_route_id=self.preview_route_id,committed_route_id=self.committed_route_id,preview_overlay=self.preview_overlay,gravity_shadow_report=self.gravity_shadow_report,campaign_state_sha256=self.campaign_state_sha256,execution_available=self.campaign_execution_service is not None,last_execution=self.last_execution)
    ...
    def preview(self,route_id):
        ...
        layer=self.service.get_route_layer(plan,self.context); self.preview_route_id=candidate.route_id; self.preview_overlay=build_navigation_overlay(layer).to_dict(); self.gravity_shadow_report=None
        try:
            ...
            if isinstance(trajectory,Mapping):
                roots=resolve_runtime_roots(app_root=getattr(self.context,"runtime_root",None)); db_path=roots.data_root/"LOOM_2226.sqlite3"; self.gravity_shadow_report=compare_live_route_trajectory(trajectory,db_path)
    ...
    def cancel(self):self.destination=None; self.priority="BALANCED"; self._request=None; self._candidates.clear(); self._plans.clear(); self.preview_route_id=None; self.committed_route_id=None; self.preview_overlay=None; self.gravity_shadow_report=None; self._assert_read_only(); return self.state()
