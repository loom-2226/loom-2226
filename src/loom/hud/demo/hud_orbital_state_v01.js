...
panel.innerHTML='<b>EARTH FLIGHT STATE</b><br>STATE UNAVAILABLE';stage.appendChild(panel);
const fmt=(n,d=1)=>Number.isFinite(Number(n))?Number(n).toFixed(d):'—';
function render(payload){
  const o=payload&&payload.orbital_state;
  if(!o){panel.innerHTML='<b>EARTH FLIGHT STATE</b><br>STATE UNAVAILABLE';return;}
  const operational=o.operational_orbit===true&&o.presentation_regime==='EARTH_ORBIT';
  const period=Number(o.period_s);
  if(!operational){
    const regime=String(o.presentation_regime||'FREE_FLIGHT_OSCULATING');
    panel.innerHTML=`<b>EARTH FREE FLIGHT • ${regime}</b><br>`+
      `R ${fmt(o.radius_km,1)} km • ALT ${fmt(o.altitude_km,1)} km<br>`+
      `V ${fmt(o.speed_km_s,3)} km/s • ECC ${fmt(o.eccentricity,5)}<br>`+
      `OSCULATING ${String(o.classification||'UNKNOWN')}<br>`+
      `AP/PE/PERIOD SUPPRESSED — NOT AN OPERATIONAL ORBIT<br>`+
  ...
  panel.innerHTML=`<b>ORBIT / EARTH • ${String(o.classification||'UNKNOWN')}</b><br>`+
    ...
    `AP ${fmt(o.apoapsis_altitude_km,1)} km • PE ${fmt(o.periapsis_altitude_km,1)} km<br>`+
