...
function rebuildOrbit(scene){
  ...
  if(!(vis.available===true&&o.operational_orbit===true&&Array.isArray(points)&&points.length>2&&Array.isArray(shipState.position_earth_centered_km))){removeOrbitGroup();lastKey='';return;}
  ...
  const line=new THREE.Line(new THREE.BufferGeometry().setFromPoints(rel),new THREE.LineBasicMaterial({color:0x7fd7d0,transparent:true,opacity:.72}));
  orbitGroup.add(line);
  function markerAt(p,label){if(!Array.isArray(p))return;const g=new THREE.Group();g.name=label;const m=new THREE.Mesh(new THREE.SphereGeometry(45,12,8),new THREE.MeshBasicMaterial({color:0xf0c36a}));m.position.copy(v3(p).sub(shipPos));g.add(m);orbitGroup.add(g);}
...
function orbitCamera(camera){
  ...
  if(cameraSel.value==='ORBIT'){
    ...
    camera.position.copy(earthRel).add(normal.multiplyScalar(extent*2.45));
    camera.up.copy(radial);camera.near=Math.max(.01,extent/10000);camera.far=Math.max(5000000,extent*20);camera.fov=55;camera.updateProjectionMatrix();camera.lookAt(earthRel);
    if(cameraState)cameraState.textContent='CAM DEFAULT / ORBIT OVERVIEW';
    return true;
  }
...
cameraSel.addEventListener('change',()=>{userCamera=false;if(cameraSel.value==='ORBIT'&&cameraState)cameraState.textContent='CAM DEFAULT / ORBIT OVERVIEW';});
