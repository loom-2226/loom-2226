...
emitted by the current allocator.  From that sampled command history it derives
requirements-facing quantities that can constrain later hardware selection:

- smallest nonzero sampled impulse demand at the present numerical cadence;
- largest sampled thrust-command step;
- largest sampled force-vector direction step;
- corresponding one-sample average direction-slew lower bound.

These are properties of the current 1 s qualification trace, not claims about
minimum impulse bit, valve latency/bandwidth, gimbal slew/acceleration, or final
hardware.  Hardware qualification remains fail-closed until evidence is supplied.
"""
...
from src.wayfarer_rcs_time_domain_mount_coupling import (
    ...
    HOLD_TIME_S,
    MAX_TIME_S,
    TRANSLATION_RESPONSE_RATE_S_INV,
    VELOCITY_GATE_M_S,
    _AXIS_INDEX,
    _CASES,
    _DEGRADED_FORCE_LIMIT_N,
    _DEGRADED_TORQUE_LIMITS,
    _NOMINAL_FORCE_LIMIT_N,
    _NOMINAL_TORQUE_LIMITS,
)
...
def requirement_authority_contract() -> dict[str, Any]:
    return {
        "sampled_flight_demand_envelope_derivable": True,
        "minimum_impulse_bit_certified": False,
        "valve_dynamics_certified": False,
        "gimbal_dynamics_certified": False,
...
def summarize_mount_command_trace(trace: list[dict[str, Any]], *, sample_period_s: float) -> dict[str, Any]:
    ...
    for row in trace:
        grouped[str(row["mount_id"])].append(row)
    ...
    for rows in grouped.values():
        ...
        for row in rows:
            ...
            if previous is not None:
                ...
                prev_dir = previous.get("force_unit")
                curr_dir = row.get("force_unit")
    ...
    return {
        ...
        "mount_count_observed": len(grouped),
        "command_samples": len(trace),
        "minimum_nonzero_sampled_impulse_Ns": minimum_impulse,
        "sampled_exact_trace_mib_upper_bound_Ns": minimum_impulse,
        "maximum_sampled_thrust_N": max_thrust,
        "maximum_sampled_thrust_step_N": max_thrust_step,
...
def build_actuator_requirement_envelope() -> dict[str, Any]:
    ...
    for name, result in case_results.items():
        trace = list(result.pop("trace"))
        summary = summarize_mount_command_trace(trace, sample_period_s=CONTROL_DT_S)
        all_trace.extend(trace)
        case_summaries[name] = {**result, "sampled_actuator_demand": summary}
    ...
    return {
        ...
        "disposition": (
            "SAMPLED_ACTUATOR_DEMAND_ENVELOPE_DERIVED_HARDWARE_UNQUALIFIED"
            if all_pass
            else "ACTUATOR_DEMAND_ENVELOPE_DERIVATION_FAIL"
        ),
        "requirement_contract": dict(REQUIREMENT_CONTRACT),
        "cases": case_summaries,
        "aggregate_sampled_actuator_demand": aggregate,
        "authority": requirement_authority_contract(),
        "remaining_open": [
            "minimum_impulse_bit_hardware_value_and_discretized_closed_loop_validation",
