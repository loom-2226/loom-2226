...
class LoomDoctorTest(unittest.TestCase):
    def test_json_mode_is_read_only(self):
        with TemporaryDirectory() as tmp:
            ...
            argv=[str(DOCTOR),"--app-root",str(app),"--data-root",str(data),"--campaign-root",str(campaign),"--json"]
            with patch.object(sys,"argv",argv), redirect_stdout(output):
                with self.assertRaises(SystemExit) as cm: runpy.run_path(str(DOCTOR),run_name="__main__")
            self.assertEqual(cm.exception.code,0); manifest=json.loads(output.getvalue())
    ...
    def test_human_mode_reports_explicit_roots(self):
        with TemporaryDirectory() as tmp:
            ...
            argv=[str(DOCTOR),"--app-root",str(app),"--data-root",str(data),"--campaign-root",str(campaign)]
            with patch.object(sys,"argv",argv), redirect_stdout(output):
                with self.assertRaises(SystemExit) as cm: runpy.run_path(str(DOCTOR),run_name="__main__")
            self.assertEqual(cm.exception.code,0); text=output.getvalue(); self.assertIn("LOOM RUNTIME AUDIT",text); self.assertIn(str(app.resolve()),text); self.assertIn(str(data.resolve()),text); self.assertIn(str(campaign.resolve()),text)
