...
class CoverageNormalizationEmbeddedSequenceHFunctionalTest(unittest.TestCase):
    ...
    def _install_plan(self, nav, scope: str, td: str):
        original_window = nav.acquisition_window
        original_plan = nav.build_acquisition_plan
        original_cache_paths = nav.cache_paths
        ...
        nav.cache_paths = lambda cache_dir, req: (Path(td) / "missing.cache", Path(td) / "missing.raw")
        return original_window, original_plan, original_cache_paths

    def test_local_coverage_gap_uses_frozen_direct_unavailable_policy(self):
        with TemporaryDirectory() as td:
            nav = self._nav(td)
            original_fetch = nav.fetch_or_cache
            original_window, original_plan, original_cache_paths = self._install_plan(nav, "LOCAL", td)
            try:
                ...
                self.assertIn("navigation_grade=false", result["entries"][0]["fallback_rule"])
            finally:
                nav.fetch_or_cache = original_fetch
                nav.acquisition_window = original_window
                nav.build_acquisition_plan = original_plan
                nav.cache_paths = original_cache_paths

    def test_base_coverage_gap_still_fails_closed(self):
        with TemporaryDirectory() as td:
            nav = self._nav(td)
            original_fetch = nav.fetch_or_cache
            original_window, original_plan, original_cache_paths = self._install_plan(nav, "BASE", td)
            try:
                nav.fetch_or_cache = lambda *a, **k: (_ for _ in ()).throw(
