...
class RuntimeRootsTest(unittest.TestCase):
    def test_explicit_roots_win(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            roots = resolve_runtime_roots(app_root=root / "app", data_root=root / "world", campaign_root=root / "campaign", env={"LOOM_APP_ROOT": "/wrong", "LOOM_HOME": "/also-wrong"}, cwd=root)
    ...
    def test_dedicated_environment_precedes_loom_home(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            roots = resolve_runtime_roots(env={"LOOM_APP_ROOT": str(root / "app"), "LOOM_DATA_ROOT": str(root / "data"), "LOOM_CAMPAIGN_ROOT": str(root / "campaign"), "LOOM_HOME": str(root / "legacy")}, cwd=root)
            self.assertEqual(roots.app_root, (root / "app").resolve())
    ...
    def test_export_sets_new_contract_and_legacy_home(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            roots = resolve_runtime_roots(app_root=root / "app", data_root=root / "data", campaign_root=root / "campaign", env={}, cwd=root)
            env = {}
