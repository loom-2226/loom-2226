...
Approval creates or reuses a named Lightsail key pair, writes a new private key only when no local key exists, uses a `077` umask and atomic `0600` file custody, and refuses to proceed when a remote key exists without its local custody file. The default firewall is private-access-first: only TCP/22 from the supplied `/32` is opened. Public HTTPS requires the additional explicit `--allow-public-https` flag. It does not install software or deploy the Atlas. The script is idempotent: an existing named instance causes it to report and exit without creating another.
