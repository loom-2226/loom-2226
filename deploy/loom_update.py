#!/usr/bin/env python3
"""LOOM 2226 updater/bootstrap v0.5.

Installs immutable application/code and canonical reference data into explicit
runtime roots. Mutable campaign state is outside updater authority and is never
...
WINDOWS_APP_ROOT=Path.home()/"Documents"/"LOOM"; WINDOWS_DATA_ROOT=WINDOWS_APP_ROOT/"data"
MANIFEST_PATH="manifests/release_manifest.json"; INSTALL_STATE=".loom_install_state.json"; DOWNLOAD_CHUNK_BYTES=1024*1024; PROGRESS_THRESHOLD_BYTES=8*1024*1024
@dataclass(frozen=True)
class InstallRoots:
    app_root:Path; data_root:Path
@dataclass(frozen=True)
class ArtifactResult:
    path:str; target:str|None; state:str; expected_sha256:str|None; actual_sha256:str|None
...
def platform_roots(app_explicit=None,data_explicit=None,environ=None):
    env=os.environ if environ is None else environ; legacy=str(env.get("LOOM_HOME","")).strip(); app_value=app_explicit or str(env.get("LOOM_APP_ROOT","")).strip() or legacy; android=Path("/storage/emulated/0").exists()
    app=Path(app_value).expanduser() if app_value else (ANDROID_APP_ROOT if android else WINDOWS_APP_ROOT)
    data_value=data_explicit or str(env.get("LOOM_DATA_ROOT","")).strip(); data=Path(data_value).expanduser() if data_value else ((ANDROID_DATA_ROOT if android else app/"data"))
    return InstallRoots(app.resolve(),data.resolve())
...
def github_bytes(url,accept,*,label=None,expected_size=None):
    req=Request(url,headers={"Authorization":f"Bearer {token()}","Accept":accept,"X-GitHub-Api-Version":"2022-11-28","User-Agent":"LOOM-2226-Updater/0.5"})
...
def github_artifact_bytes(artifact,ref):
    source=artifact.get("source","repository"); label=artifact["path"]; size=artifact.get("size_bytes")
    if source=="repository":return github_bytes(f"{CONTENTS_API}/{artifact['path']}?ref={ref}","application/vnd.github.raw+json",label=label,expected_size=size)
    if source=="release_asset":
        aid=artifact.get("asset_id")
        if not isinstance(aid,int):raise RuntimeError(f"Release asset missing numeric asset_id: {artifact['path']}")
...
def target_for(roots,artifact):
    roots=_coerce_roots(roots); src=Path(artifact["path"]); group=artifact["install_group"]
    if group=="code":return roots.app_root/"src"/src.name
    if group in {"canonical_data","media"}:return roots.data_root/src.name
    raise ValueError(f"Unknown install_group: {group}")
def atomic_write(target,data,backup_root=None):
    ...
    try:
        if target.exists() and backup_root is not None:
            backup=backup_root/target.name; backup.parent.mkdir(parents=True,exist_ok=True); shutil.copy2(target,backup)
...
def validate_local(roots,manifest):
    ...
    for a in manifest["artifacts"]:
        expected=a.get("sha256"); target=target_for(roots,a)
        ...
        actual=sha256_file(target); size=a.get("size_bytes"); out.append(ArtifactResult(a["path"],str(target),"OK" if actual==expected and (size is None or target.stat().st_size==size) else "MISMATCH",expected,actual))
    return out
def install_release(roots,manifest,ref,artifact_fetcher=github_artifact_bytes,dry_run=False):
    ...
    for a in manifest["artifacts"]:
        expected=a.get("sha256"); required=bool(a.get("required")); target=target_for(roots,a); print(f"CHECKING         {a['path']}")
        ...
        if target.exists() and (a.get("size_bytes") is None or target.stat().st_size==a["size_bytes"]):
            ...
            if sha256_file(target)==expected:out.append(ArtifactResult(a["path"],str(target),"UNCHANGED",expected,expected)); continue
        ...
        print(f"VERIFYING        {a['path']}"); actual=sha256_bytes(payload)
        if actual!=expected:raise RuntimeError(f"HASH MISMATCH for {a['path']}\nexpected={expected}\nactual={actual}")
        if not dry_run:atomic_write(target,payload,backup_root=backup_root); state="INSTALLED"
