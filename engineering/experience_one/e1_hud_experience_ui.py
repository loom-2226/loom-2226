...
def human_location(token: str) -> str:
    """Render an authoritative location token as player-facing text only."""
...
def render_experience_html(diagnostic_html: str, *, location: str, ship_name: str) -> str:
    ...
    place = escape(human_location(location))
...
.e1-details{margin-top:18px;border-top:1px solid var(--line);padding-top:14px;max-width:100%}
...
.panel,.intent-result,.navigator-request,.review,.final-plan,.authorization,.execution-request,.execution{min-width:0;max-width:100%;overflow-wrap:anywhere}
.intent-result,.navigator-request,.review,.final-plan,.authorization,.execution-request,.execution{margin-top:14px}
.final-plan code,.final-plan pre,.technical-details{max-width:100%;overflow-wrap:anywhere;word-break:break-word}
.technical-details{margin-top:12px;color:var(--muted)}
.technical-details summary{cursor:pointer;font-size:12px}
.technical-details pre{white-space:pre-wrap;overflow-x:auto;font-size:11px;line-height:1.4}
@media(max-width:760px){
main{padding-top:18px}
.e1-world-hero{padding-top:8px}
.review{overflow:visible}
.review table{min-width:0!important;max-width:100%;width:100%;display:block}
.review thead{display:none}
.review tbody{display:block;width:100%}
.review tr{display:block;width:100%;border:1px solid var(--line);margin:10px 0;padding:8px 10px;background:#0c1118}
.review td{display:grid;grid-template-columns:7.1rem minmax(0,1fr);gap:10px;align-items:center;width:100%;border-top:1px solid var(--line);padding:8px 0;white-space:normal;overflow-wrap:anywhere}
