...
"""E1.1 Mara grounded-synthesis empirical harness v0.6.

v0.6 corrects an over-constrained empirical oracle exposed by the v0.5 Pixel run.
Some compound natural-language questions admit more than one epistemically valid
assessment while preserving the same deterministic evidence boundary.

This harness therefore validates membership in a small preregistered acceptable
assessment set per case rather than forcing one scalar label where the proposition
is compound. It does NOT relax evidence-path validation, canonical target identity,
intent selection, authority boundaries, or the requirement for manual semantic
review of the prose.
...
DEFAULT_OUT = Path("/storage/emulated/0/Download/E1_1_MARA_SYNTHESIS_RESULT_V06.json")
DEFAULT_AUDIT = Path("/storage/emulated/0/Download/LOOM_OPENAI_DEV_AUDIT.jsonl")
ASSESSMENTS = v05.ASSESSMENTS
SYSTEM = v05.SYSTEM
...
CASES = [
    ...
    },
    {
        ...
        "expected_target": "CER-P05",
        "acceptable_assessments": {"SUPPORTED", "MIXED"},
    ...
    },
    {
        ...
        "expected_intent": "PLACE_DETAIL",
        "expected_target": "CER-P05",
        "acceptable_assessments": {"NOT_ESTABLISHED"},
    ...
    },
    {
        ...
        "expected_intent": "PLACE_DETAIL",
        "expected_target": "CER-P05",
        "acceptable_assessments": {"CONTRADICTED", "MIXED"},
...
def packet_path_exists(packet: dict[str, Any], path: str) -> bool:
    return v05.packet_path_exists(packet, path)
...
def validate_answer(case: dict[str, Any], packet: dict[str, Any], answer: dict[str, Any]) -> tuple[bool, list[str]]:
    ...
    acceptable = set(case.get("acceptable_assessments") or set())
    if not acceptable:
        reasons.append("acceptable_assessments_missing")
    elif assessment not in acceptable:
        reasons.append("assessment_outside_acceptable_set")
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema": "LOOM_E1_1_MARA_GROUNDED_SYNTHESIS_RESULT_V06",
        "model": args.model,
        "authority": {
            ...
            "assessment_validation": "PREREGISTERED_BOUNDED_ACCEPTABLE_SET",
    ...
    for case in CASES:
        ...
        try:
            r1, t1 = base.api_post(key, p1, args.timeout, audit, stage=f"{cid}.request1_tool_call_v06", metadata={"case": cid, "phase": "MODEL_TO_TYPED_REFERENCE"})
        ...
        try:
            r2, t2 = base.api_post(key, p2, args.timeout, audit, stage=f"{cid}.request2_grounded_synthesis_v06", metadata={"case": cid, "phase": "TYPED_QUERY_TO_MODEL"})
            ...
            entry = {
                ...
                "acceptable_assessments": sorted(case["acceptable_assessments"]),
            ...
            print(f"{cid:<28} {'PASS' if passed else 'FAIL'}  {t1+t2:.2f}s  assessment={parsed.get('assessment')!r}  acceptable={sorted(case['acceptable_assessments'])}  reasons={reasons}")
        except Exception as exc:
            entry = {"case": cid, "user_prompt": case["prompt"], "acceptable_assessments": sorted(case["acceptable_assessments"]), "raw_tool_arguments": raw_args, "normalized_tool_arguments": normalized, "tool_packet": packet, "pass": False, "fail_reasons": [f"request2_or_parse_error:{type(exc).__name__}:{exc}"]}
