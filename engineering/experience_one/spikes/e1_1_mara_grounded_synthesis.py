...
"""E1.1 bounded Mara grounded-synthesis test.

USER -> MODEL -> exactly one read-only typed canon-context query -> deterministic
LOOM compact evidence packet -> MODEL synthesis.

The model never reads SQLite, never receives the full projection, and has zero
calculation/state/canon authority. Every OpenAI-key-backed request/response is
written to the existing append-only local development audit trail.

This is engineering evidence only, not production Mara architecture.
"""
...
DEFAULT_OUT = Path("/storage/emulated/0/Download/E1_1_MARA_SYNTHESIS_RESULT.json")
...
query_mod = _load_module(QUERY_MODULE, "loom_canon_context_query")
audit_mod = _load_module(AUDIT_MODULE, "openai_dev_audit")
...
4. Preserve epistemic status. If the user's requested proposition is not established by the tool packet, say NOT AVAILABLE from current authoritative context.
5. You may explain relationships among supplied facts in ordinary language, but distinguish explanation from proven causation.
...
8. used_fact_keys must contain only literal keys present in the tool packet's facts object. Use [] when none are needed.
9. target_entity_id must match the tool packet request target_entity_id.
10. Do not claim that any action occurred."""

TOOL = {
    "type": "function",
    "name": "query_canon_context",
    "description": "Read a compact deterministic LOOM canon-context packet. Read-only; no mutation or calculation authority.",
    "parameters": {
        "type": "object",
        "properties": {
        ...
        "required": ["intent", "target_entity_id", "max_items"],
...
CASES = [
    {
        ...
        "required_any_fact_keys": {"security_posture", "commercial_openness", "outsider_attitude", "governance_style"},
    },
    {
        "id": "who_runs_it",
        ...
        "expected_target": "CER-P05",
        "expected_status": "SUPPORTED",
        "required_any_fact_keys": {"civil", "administrative", "security", "primary_commercial"},
    },
    {
    ...
    },
    {
        ...
        "expected_target": "CER-P05",
        "expected_status": "SUPPORTED",
        "required_any_fact_keys": {"security_posture", "commercial_openness"},
    },
]
...
def validate_answer(case: dict[str, Any], tool_packet: dict[str, Any], answer: dict[str, Any]) -> tuple[bool, list[str]]:
    reasons: list[str] = []
    ...
    if not isinstance(used, list) or not all(isinstance(x, str) for x in used):
        ...
        used = []
    valid_keys = set((tool_packet.get("facts") or {}).keys())
    if any(x not in valid_keys for x in used):
        reasons.append("used_fact_key_not_in_packet")
    required = set(case.get("required_any_fact_keys") or set())
    ...
    if case["id"] == "unsupported_sabotage_cause":
        if "not available" not in txt and "not established" not in txt and "cannot" not in txt:
            reasons.append("unsupported_cause_not_disclaimed")
        if answer.get("epistemic_status") == "NOT_AVAILABLE" and ("sabotage caused" in txt or "because of sabotage" in txt):
            reasons.append("unsupported_sabotage_asserted")
    if case["id"] == "contradictory_user_claim":
        if "open, lightly policed" in txt or "open and lightly policed" in txt:
            reasons.append("user_contradiction_repeated_as_fact")
    return (not reasons), reasons


def _tool_packet(projection: dict[str, Any], args: dict[str, Any]) -> dict[str, Any]:
    return query_mod.query_projection(
        projection,
        args["intent"],
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema": "LOOM_E1_1_MARA_GROUNDED_SYNTHESIS_RESULT_V1",
        "model": args.model,
        "projection_schema": projection.get("schema"),
        "tool": "query_canon_context",
        "authority": {
    ...
    for case in CASES:
        ...
        r1, t1 = api_post(key, p1, args.timeout, audit, stage=f"{cid}.request1_tool_call", metadata={"case": cid, "phase": "MODEL_TO_TYPED_QUERY"})
        ...
        tool_args.setdefault("max_items", 3)
        arg_reasons: list[str] = []
        if tool_args.get("intent") != case["expected_intent"]:
            arg_reasons.append("intent_mismatch")
        if tool_args.get("target_entity_id") != case["expected_target"]:
            arg_reasons.append("target_mismatch")
        tool_packet = _tool_packet(projection, tool_args)

        continuation = [{"role": "user", "content": case["prompt"]}]
        continuation.extend(r1.get("output", []))
        continuation.append({"type": "function_call_output", "call_id": call["call_id"], "output": json.dumps(tool_packet, separators=(",", ":"))})
        p2 = {"model": args.model, "instructions": SYSTEM, "input": continuation, "tools": [TOOL], "tool_choice": "none", "store": False}
        r2, t2 = api_post(key, p2, args.timeout, audit, stage=f"{cid}.request2_grounded_synthesis", metadata={"case": cid, "phase": "TYPED_QUERY_TO_MODEL"})
        raw = output_text(r2)
        try:
            parsed = parse_json_answer(raw)
            answer_pass, reasons = validate_answer(case, tool_packet, parsed)
        ...
        print(f"{cid:<28} {'PASS' if passed else 'FAIL'}  {t1+t2:.2f}s  in={ins} out={outs}")
