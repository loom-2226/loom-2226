...
load_qualification_pointer_config() {
  ...
  if ! remote_config="$(git show "$QUALIFICATION_POINTER_REF:$CONFIG_REL" 2>/dev/null)"; then
...

{
  ...
  git fetch origin; choose_bootstrap_config
  ...
  [[ -n "$BOOTSTRAP_SPATIAL_REVIEW_COMMAND" ]] && echo "BOOTSTRAP_SPATIAL_REVIEW_COMMAND=$BOOTSTRAP_SPATIAL_REVIEW_COMMAND"
  echo "[2/4] SYNC GOVERNED ACTIVE BRANCH"; switch_to_config_branch
  echo "[3/4] FAST-FORWARD + CONFIG REFRESH"; git pull --ff-only; load_config "POST_PULL"
  CURRENT_BRANCH="$(git branch --show-current)"
  if [[ "$QUAL_BRANCH" != "$CURRENT_BRANCH" ]]; then
    echo "CONFIG_HANDOFF=$CURRENT_BRANCH->$QUAL_BRANCH"; switch_to_config_branch; git pull --ff-only; load_config "POST_HANDOFF_PULL"
    CURRENT_BRANCH="$(git branch --show-current)"
  ...
  [[ -n "$SPATIAL_REVIEW_COMMAND" ]] && echo "EFFECTIVE_SPATIAL_REVIEW_COMMAND=$SPATIAL_REVIEW_COMMAND"
...
if [[ $PIPE_RC -eq 0 && -n "$SPATIAL_REVIEW_COMMAND" ]]; then
  {
    ...
    nohup bash -lc "$SPATIAL_REVIEW_COMMAND" >"$REVIEW_LOG" 2>&1 & REVIEW_PID=$!; sleep 0.5
...
echo; echo "QUALIFICATION_LOG_SAVED=$QUALIFICATION_LOG"; echo "QUALIFICATION_LOG_LATEST=$LATEST_QUALIFICATION_LOG"

if command -v termux-clipboard-set >/dev/null 2>&1; then
  termux-clipboard-set < "$TMP"; echo; echo "Complete qualification output copied to Android clipboard."
else
  echo; echo "WARNING: termux-clipboard-set unavailable; output was not copied." >&2
fi
