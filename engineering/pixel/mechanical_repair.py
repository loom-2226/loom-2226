...
def _response_schema() -> dict:
    return {
        "type": "object",
        "properties": {
            "classification": {"type": "string", "enum": ["MECHANICAL_REPAIRABLE", "ESCALATE"]},
            "summary": {"type": "string"},
            "reason": {"type": "string"},
            "edits": {
                "type": "array",
...
"""
    payload = {
        ...
        "text": {
            "verbosity": "low",
            "format": {"type": "json_schema", "name": "loom_mechanical_repair", "strict": True, "schema": _response_schema()},
        },
    }
    req = urllib.request.Request(API_URL, data=json.dumps(payload).encode("utf-8"), headers={"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}, method="POST")
    try:
...
def main() -> int:
    ...
    try:
        ...
        decision = {"classification": "ESCALATE", "summary": "Local mechanical-repair agent could not produce a safe candidate.", "reason": str(exc), "edits": [], "applied_paths": [], "attempt": args.attempt, "model": os.environ.get("LOOM_REPAIR_MODEL", DEFAULT_MODEL)}
