...
function drawBody(view,back){
 ...
 if(domain==='world'){
  const hero=element('figure',undefined,'hero-ceres');const heroImage=element('img');const heroFallback=element('span','Approved Ceres WORLD HERO unavailable. No substitute is shown.','image-fallback');heroImage.alt='Ceres — approved WORLD reference image';heroFallback.hidden=true;heroImage.addEventListener('error',()=>{heroImage.hidden=true;heroFallback.hidden=false});heroImage.addEventListener('load',()=>{heroImage.hidden=false;heroFallback.hidden=true});heroImage.src='assets/ceres-world-hero.png';hero.append(heroImage,heroFallback,element('figcaption','HERO · APPROVED_REFERENCE · asset 0e516224-6345-4e97-b6d1-569bf193f495'));identity.append(hero,title,element('p','The physical world. The inhabited world.','lede'));
