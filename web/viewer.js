...
var stage=document.getElementById('stage'),panel=document.getElementById('panel'),panelBody=document.getElementById('panelBody'),panelClose=document.getElementById('panelClose'),status=document.getElementById('status'),validation=document.getElementById('validation');
var selectedComponentId=null;

function closePanel(){selectedComponentId=null;panel.hidden=true;panelBody.textContent='';}
function showPanel(component){selectedComponentId=component.id;panelBody.textContent=JSON.stringify(component,null,2);panel.hidden=false;}
...
try{
  ...
  function setView(v){closePanel();target.set(28,0,0);if(v==='side'){theta=0;phi=.04;distance=85;}else if(v==='top'){theta=0;phi=1.45;distance=85;}else if(v==='front'){theta=Math.PI;phi=.04;target.set(0,0,0);distance=70;}else if(v==='aft'){theta=0;phi=.04;target.set(57,0,0);distance=70;}else{theta=.55;phi=.65;distance=85;}updateCamera();}
